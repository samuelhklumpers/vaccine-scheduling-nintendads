    ...
    class LinearProgramming
    {
        public static void Solve(OfflineProblem problem)
        {
            ...
            foreach (var variable in solver.variables())
            {
                ...
                if (data[0][0] == 'z')
                {
                    resulting_matrix_z[int.Parse(data[1]), int.Parse(data[2])] = variable.SolutionValue();
                }
            }
            ...
            foreach (var variable in solver.variables())
            {
                ...
                if (data[0][0] == 'c')
                {
                    resulting_matrix_compare[int.Parse(data[1]), int.Parse(data[2])] = variable.SolutionValue();
                }
            }
            ...
            foreach (var variable in solver.variables())
            {
                ...
                if (data[0][0] == 'y' )
                {
                    ...
                    //If we haven't counted any hospitals yet, we can add the first one
                    /*if(hospitals_used.Count() == 0)
                    {
                        hospitals_used.Add((int)variable.SolutionValue());
                    }

                    //Otherwise only add hospitals if they are different from one already used
                    bool in_list = false;
                    for(int i = 0; i < hospitals_used.Count(); i++)
                    {
                        if((int)variable.SolutionValue() == hospitals_used[i])
                        {
                            in_list = true;
                            break;
                        }
        ...
        static private Variable[] init_variables_vector(Solver solver, int length, int ub, String name)
        {
            ...
            {
                vector[i] = solver.MakeNumVar(0, ub, name + i.ToString()); //TODO check if this can be relaxed
            }
            return vector;
        }
