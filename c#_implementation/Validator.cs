...
namespace implementation
{
    class OfflineValidator
    {
        public void validate(OfflineProblem problem, Solution solution) {
            if (solution.withHospital)
            {
                if (problem.number_of_patients != solution.registrationWithHospitals.Count) {
                    throw new Exception($"solution and problem have differing numbers of patients: #regs={solution.registrationWithHospitals.Count}, #patients={problem.number_of_patients}");
                }
            }
            else {
